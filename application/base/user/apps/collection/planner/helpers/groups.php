...
class Groups {
    ...
    public function save_group() {
        ...
        if ($this->CI->input->post()) {
            ...
            if ( $this->CI->form_validation->run() == false ) {
                ...
                // Create a new group
                $save_group = $this->CI->planner_lists_model->save_group( $this->CI->user_id, 'social', $group_name, '' );
    ...
    public function available_group_accounts() {
        ...
        if ( $group_id ) {
            ...
            // Get group's metas
            $group_metas = $this->CI->planner_lists_model->get_lists_meta( $this->CI->user_id, $group_id );
    ...
    public function delete_group() {
        ...
        if ( $group_id ) {
            ...
            // Delete Group
            $group_delete = $this->CI->planner_lists_model->delete_list( $this->CI->user_id, $group_id, 'social' );
    ...
    public function add_account() {
        ...
        if ( is_numeric($account_id) && is_numeric($group_id) ) {
            ...
            $owner = $this->CI->planner_lists_model->if_user_has_list( $this->CI->user_id, $group_id, 'social' );
            
            // Get the account data
            $get_account = $this->CI->planner_networks_model->get_account( $account_id );
            
            // Verify if the account was added before in the group
            $if_account_exists = $this->CI->planner_lists_model->if_item_is_in_list( $this->CI->user_id, $group_id, $account_id );
            
            if ( ( $owner === TRUE ) && ( $get_account[0]->user_id === $this->CI->user_id ) && ( $if_account_exists === FALSE) ) {
                
                // Save the account
                $save = $this->CI->planner_lists_model->save_group_account( $group_id, $this->CI->user_id, $account_id );
                
                // Verify if the account was saved
                if ( $save ) {
                    
                    $accounts = $this->CI->planner_lists_model->get_lists_meta( $this->CI->user_id, $group_id );
    ...
    public function remove_account() {
        ...
        if ( is_numeric($account_id) && is_numeric($group_id) ) {
            ...
            if ( ( $owner === TRUE ) && ( $get_account[0]->user_id === $this->CI->user_id ) && ( $if_account_exists === TRUE) ) {
                ...
                // Remove the account
                $remove = $this->CI->planner_lists_model->remove_group_account( $group_id, $this->CI->user_id, $account_id );
